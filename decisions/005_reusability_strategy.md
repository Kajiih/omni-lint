...
While tools like `ast-grep` and `semgrep` exist for parsing code, they do not support orchestrating Command Guards or Environment Context. Our package acts as a unified orchestrator (sharing config and reporting via `src/core.rs`) to bring these disparate domains under a single `.toml` configuration, while deploying them as separate, focused binaries (`omni-lint` and `omni-guard`).
...
### 1. Diagnostics & Reporting (`src/core.rs`)
...
### 2. Code Parsing & Static Analysis (`src/code.rs`)
...
### 3. Command Validation (`src/cmd.rs`)
...
### 4. Environment & Context Guards (`src/env.rs`)
