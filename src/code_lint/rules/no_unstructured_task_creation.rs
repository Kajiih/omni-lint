...
mod tests {
    ...
    fn test_unstructured_calls_flagged() {
        ...
        [no-unstructured-task-creation] Line 4, Col 10: Unstructured task creation `asyncio.create_task()` is discouraged.
        [no-unstructured-task-creation] Line 5, Col 10: Unstructured task creation `asyncio.ensure_future()` is discouraged.
        [no-unstructured-task-creation] Line 6, Col 10: Unstructured task creation `create_task()` is discouraged.
        [no-unstructured-task-creation] Line 7, Col 10: Unstructured task creation `ensure_future()` is discouraged.
        [no-unstructured-task-creation] Line 8, Col 10: Unstructured task creation `loop.create_task()` is discouraged.
        [no-unstructured-task-creation] Line 9, Col 10: Unstructured task creation `event_loop.create_task()` is discouraged.
        [no-unstructured-task-creation] Line 10, Col 10: Unstructured task creation `asyncio.get_event_loop().create_task()` is discouraged.
        ");
    }
    ...
    fn test_config_extend_and_allowed() {
        ...
        insta::assert_snapshot!(output, @"[no-unstructured-task-creation] Line 2, Col 5: Unstructured task creation `custom_scheduler.spawn_background()` is discouraged.");
    }
}
